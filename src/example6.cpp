...
std::pair<long long, bool> is_prime_fast(std::future<long long> n) {
  auto val = n.get();
  std::set<long long> known_primes {3, 5, 7, 11, 13, 17, 23, 29, 31, 37};
  if (val % 2 == 0) return std::make_pair(val, false);
  for (const auto& p : known_primes) {
    if (val % p == 0) return std::make_pair(val, false);
  }
  for (long long i = 37; i < static_cast<long long>(std::sqrt(val)); i += 2) {
    if (val % i == 0) return std::make_pair(val, false);
  }
...
int main() {
  ...
  while (!prime_q.empty()) {
    auto f = std::async(std::launch::async, is_prime_fast, std::move(prime_q.front()));
